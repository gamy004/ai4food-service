...
export class SwabProductManagerService {
    ...
    async generateSwabProductPlan(generateSwabProductPlanDto: GenerateSwabProductPlanDto) {
        const { fromDate, toDate, roundNumberSwabTest = "" } = generateSwabProductPlanDto;
        ...
        for (let index = 0; index < facilitysTemplate.length; index++) {
            const { facilityName, mainFacilityItems = [], swabPeriodMapping = [], shiftMapping = [] } = facilitysTemplate[index];

            const fetchFacilityItems = await Promise.all(mainFacilityItems.map(
                async (mainFacilityItem) => {
                    const facilityItem = await this.facilityItemRepository.findOne({
                        where: {
                            facilityItemName: mainFacilityItem.facilityItemName,
                        }
                    });
            ...
            facilityItems.push(fetchFacilityItems)
        }

        async function generateSwabProductHistory(swabProductDate, facilityItem, product, swabPeriod, shift = null, createSwabTest = true) {
            const historyData = {
            ...
            return swabProductHistories.push(swabProductHistory);
        }

        async function generateHistory(facilityItemsAll, currentDate = new Date(), dateIndex) {
            ...
            if (dateIndex === 0) {
                for (let index = 0; index < bigCleaningSwabPeriodsTemplate.length; index++) {
                    ...
                    for (let index = 0; index < facilityItemsAll.length; index++) {
                        const facilityItemGroupByFacility = facilityItemsAll[index];

                        for (let indexfacilityItem = 0; indexfacilityItem < facilityItemGroupByFacility.length; indexfacilityItem++) {
                            const facilityItem = facilityItemGroupByFacility[indexfacilityItem];

                            for (let indexProduct = 0; indexProduct < generalProductTemplate.length; indexProduct++) {
                                const product = generalProduct[generalProductTemplate[indexProduct].productName];

                                await generateSwabProductHistory(
                                    currentDate,
                                    facilityItem,
                                    product,
            ...
            for (let index2 = 0; index2 < shiftKeys.length; index2++) {
                ...
                for (let index = 0; index < generalSwabPeriodsTemplate.length; index++) {
                    ...
                    for (let index3 = 0; index3 < facilityItemsAll.length; index3++) {
                        const facilityItemGroupByFacility = facilityItemsAll[index3];

                        for (let index = 0; index < facilityItemGroupByFacility.length; index++) {
                            const facilityItem = facilityItemGroupByFacility[index];

                            const { swabPeriodMapping = [] } = facilityItem;
